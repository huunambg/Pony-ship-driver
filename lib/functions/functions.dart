...
streamAdminchat() async {
  chatStream = FirebaseDatabase.instance
    ...
    if (value['from_id'].toString() != userDetails['user_id'].toString() &&
        '${value['from_id'].toString()}_${value['message'].toString()}' !=
            '${adminChatList[adminChatList.length - 1]['from_id'].toString()}_${adminChatList[adminChatList.length - 1]['message'].toString()}') {
      adminChatList.add(jsonDecode(jsonEncode(event.snapshot.value)));
      unSeenChatCount =
          jsonDecode(jsonEncode(event.snapshot.value))['count'].toString();
    }
...
getadminCurrentMessages() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      unSeenChatCount = jsonDecode(response.body)['data']['count'].toString();
